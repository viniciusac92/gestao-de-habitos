...
export const GroupProvider = ({ children }) => {
  ...
  const handleGoals = title => {
    ...
    api.patch(`/goals/${currentGoal.id}/`, newGoal)
      .then(()=>setGoals([]))
      .catch((err)=>console.log(err))
  }

  const handleActivities = title => {
    const data = {title: title, realization_time: new Date().toISOString(), group:26}
  ...
  return (
    <GroupContext.Provider value={{ group, goals, activities, average, handleGoals, handleActivities }}>
      {children}
